...
/*   Updated: 2022/06/01 19:53:28 by seonkim          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_bool
get_wall_intersection(t_var *var, double ray, t_dir* wdir, double* wx, double* wy )
{
...
double
cast_single_ray(t_var *var, int x, t_dir *wdir, double *wx, double *wy)
{
	...
	draw_line(&var->image, var->game.m_py, var->game.m_px, *wy * (double)PIXEL_SIZE, *wx * (double)PIXEL_SIZE, RAY_COLOR);
    return wdist;
}
...
int main_loop(t_var *var)
{
	...
	for( int x=0; x<SCREEN_WIDTH; x++ ) {
        t_dir wdir;
		double wx, wy;
        double wdist = cast_single_ray(var, x, &wdir, &wx, &wy);
		double text_ratio = (wdir == DIR_WEST || wdir == DIR_EAST) ? (wy - floor(wy)) : (wx - floor(wx));
		int	tx = (int)(text_ratio * var->texture[wdir].texture_width);

		// int color = var->texture[wdir].texture_image[(int)(64.0 * wx) + (int)wy];
		// double lum = get_luminosity(var, wdist);
		// color = fade_color(color, lum);
        draw_wall(var, wdist, x, tx, wdir);
    }
